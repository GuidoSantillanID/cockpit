...
Before dispatching reviewers, exclude auto-generated files from the
diff handed to each agent. Bugs cannot exist in generated code —
they exist in the generator or the upstream spec, neither of which
is the subject of this review. Reviewing generated output burns
tokens and yields false findings about "hand-rolled" shape.

**Detection.** A file is generated if ANY of:

- Filename matches `**/*.generated.*`, `**/generated.{ts,js,go,py}`,
  `**/*_pb.{go,ts,py}`, `**/*.pb.go`, `**/schema.graphql.ts`
- Filename is a lockfile: `package-lock.json`, `pnpm-lock.yaml`,
  `yarn.lock`, `Cargo.lock`, `go.sum`, `Pipfile.lock`, `poetry.lock`
- First 5 lines of the file contain any of: `auto-generated`,
  `AUTO-GENERATED`, `DO NOT EDIT`, `Code generated by`, `@generated`,
  `This file is generated`

**Handling.** Emit the excluded list under `envelope.scope.excluded_files`.
Report the count in the pre-flight checklist. Reviewer prompts receive
the diff with those files stripped. User may override in pre-flight
if a specific generated file needs review (e.g. the generator config
itself changed shape).

**Example.** Run 3 evidence: a 266-file diff included
`types/generated.ts` (4065 lines) from `openapi-typescript`. That
single file was roughly half the diff body by line count. Reviewing
it produced zero findings and consumed reviewer tokens that could
have gone to the human-authored files.
...
- **Blast radius of intent-consistent refactors.** When the diff is a
  refactor (endpoint renames, API version bumps, module splits,
  signature changes), audit every caller of the changed symbols — not
  just the changed files. Intent-alignment blinds you to downstream
  breakage: the refactor reads clean, but callers break silently.
  Run 3 evidence: aligned missed 3/3 BLOCKINGs on a v4 endpoint
  migration because the refactor itself was consistent; the bugs
  lived in unchanged callers of renamed endpoints (adversarial lens
  caught all three).
...
1. **Verify.** Every finding must include `verification_command`. Run
   ...
   (reason: `missing_verification_command`). Drop findings whose
   `message` implies a pattern ("across all X", "every Y", "systemic",
   "N+ files", "throughout the codebase") but lack
   `pattern_scope.files` (reason: `missing_enumeration`). For
   import-reachability verifications that rely on grep, read the
   actual barrel chain (`index.ts` files exporting via `export * from`)
   before trusting a negative grep — see §Mandatory rules.
...
```json
{
  ...
  "scope": {
    ...
    "files_changed": 148,
    "excluded_files": ["apps/deepchain/types/generated.ts", "pnpm-lock.yaml"],
  ...
  "dropped": [
    {
      ...
      "reason": "verification_failed | missing_verification_command | missing_enumeration | category_cap | not_succinct | not_accurate | not_actionable"
    }
  ]
}
```
...
imports, reflection, interface dispatch, and barrel re-exports
(`export * from '...'` chains in `index.ts` aggregators) hide code
from grep. For import-reachability claims, traverse the barrel chain
before asserting unreachability.

Under-scoping: if your finding flags a pattern (message contains
"across all X", "every Y", "systemic", "N+ files", "throughout the
codebase"), you MUST run a grep across `path_filter` and emit the
result as `pattern_scope`: `{ grep_command, files: [all matches] }`.
Findings whose wording implies a pattern but omit `pattern_scope.files`
are dropped at the judge as `missing_enumeration`.
...
```json
{
  ...
  "pattern_scope": {
    "grep_command": "required when message implies a pattern — see §Mandatory rules",
    "files": ["all matches in path_filter"]
  }
}
```

`pattern_scope` is optional for single-site findings. Required
(enforced at the judge) when the finding's message implies a pattern.

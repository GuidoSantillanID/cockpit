...
Config file: `~/.tmux.conf` (see `tmux/tmux.conf`)
...
Config file: `~/.claude/settings.json` (see `claude/settings.json`)
...
Config files: `~/.claude/ccline/` (see `claude/ccline/`)
...
Config file: `~/shell-functions.zsh` (backup: `shell-functions.zsh`)
...
```bash
echo 'source ~/shell-functions.zsh' >> ~/.zshrc
